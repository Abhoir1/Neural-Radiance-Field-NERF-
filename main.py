...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            vis_grid_img = vis_grid(xy_grid, image_size)
            plt.imsave('images/1.3_xygrid.jpg', vis_grid_img)

        # TODO (1.3): Visualize rays using vis_rays
        if cam_idx == 0 and file_prefix == '':
            vis_rays_img = vis_rays(ray_bundle, image_size)
            plt.imsave('images/1.3_rays.jpg', vis_rays_img)
        
        # TODO (1.4): Implement point sampling along rays in sampler.py
        sampling = model.sampler(ray_bundle)

        # TODO (1.4): Visualize sample points as point cloud
        if cam_idx == 0 and file_prefix == '':
            render_points('images/1.4_sampling_point.jpg', ray_bundle.sampling.reshape(1, -1, 3))
        ...
        if cam_idx == 2 and file_prefix == '':
            ...
            depth = np.array(out['depth'].detach().cpu().view(Width, Height))
            plt.imsave("images/1.5_depth_vis.png", depth)
...
def train(
    ...
    imageio.mimsave('images/part_2x.gif', [np.uint8(im * 255) for im in all_images], loop=0)
