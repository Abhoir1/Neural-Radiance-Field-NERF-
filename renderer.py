...
class VolumeRenderer(torch.nn.Module):
    ...
    def _compute_weights(
        ...
        num_samples, num_depths, _ = deltas.shape
        transformation = [torch.ones((num_samples, 1)).cuda()]

        for i in range(1, num_depths):
            previous_transformation = transformation[i - 1]
            transformation.append(previous_transformation * torch.exp(-rays_density[:, i - 1] * deltas[:, i - 1] + eps))


        # TODO (1.5): Compute weight used for rendering from transmittance and alpha
        weights = torch.stack(transformation, dim=1) * (1-torch.exp(-rays_density*deltas+eps))
        return weights
