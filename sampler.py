...
class StratifiedRaysampler(torch.nn.Module):
    ...
    def forward(
        ...
        num_depth_values = z_vals.shape[0]

        origins = ray_bundle.origins.unsqueeze(1).repeat(1, num_depth_values, 1)
        directions = ray_bundle.directions.unsqueeze(1).repeat(1, num_depth_values, 1)

        z_vals = z_vals.unsqueeze(0).unsqueeze(-1).repeat(num_rays, 1, 1)
